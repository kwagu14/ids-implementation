...
int main(void)
{
  ...
  //check for modified blocks once at startup
  ...
  while (1){
	  //check memory over and over (this is what needs to be in IDS to monitor over and over for changed memory)
	  //SECURE_Send_Modified_Mem();
	  //send full dumps over and over
	  //SECURE_Send_Mem();

//	  CODE FOR IR IOT APP; uses ESP32 to send data over wifi to server
